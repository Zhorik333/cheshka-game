...
import { getMenuLayout } from '../systems/menuLayout';
...
export class MenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, 190, 'Будванский дворик ждёт маленькую беглянку 🐾', {
      fontFamily: 'Arial, sans-serif',
      fontSize: `${layout.subtitleFontPx}px`,
      color: '#6b4f3b',
      align: 'center',
      wordWrap: { width: layout.textWrapWidth },
    }).setOrigin(0.5);

    this.createButton(width / 2, height / 2 + 85, 'Играть', () => this.scene.start('GameScene'), 0x79b66a, layout.buttonWidth, layout.buttonHeight);
    this.createButton(width / 2, height / 2 + 165, 'Как играть', () => this.scene.start('HowToScene'), 0x6db7c8, layout.buttonWidth, layout.buttonHeight);

    this.add.text(width / 2, height - 70, 'Днём срывай объявления, ночью избегай фонариков.', {
      fontFamily: 'Arial, sans-serif',
      fontSize: `${layout.footerFontPx}px`,
      color: '#4d2c1d',
      align: 'center',
      wordWrap: { width: layout.textWrapWidth },
    }).setOrigin(0.5);
  ...
  private createButton(x: number, y: number, text: string, onClick: () => void, color: number, width: number, height: number): void {
